...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function tagsFor(item) {
    ...
    if (!ABSENT.test(item.recovery)) tags.push("회복기");
    if (/재활인증 확인/.test(item.workersComp)) tags.push("산재 재활인증");
  ...
  function openDetail(item) {
    ...
    const recoveryBadge = isRecoveryHospital 
      ? `<span class="badge" style="background: var(--amber); color: var(--white); font-weight: 800; font-size: 11px; padding: 4px 10px; margin-bottom: 8px; display: inline-block; border-radius: 6px; letter-spacing: 0;">💡 보건복지부 지정 회복기 재활기관</span>` 
      : "";
